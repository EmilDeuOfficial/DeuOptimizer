...
:: ============================================================
cls
powershell -NoProfile -Command "$lo='OS:'; $lc='CPU:'; $lg='!SI_GPU!'.Trim(); $lrf='!SI_RAMF!'.Trim(); $lrt='!SI_RAMT!'.Trim(); $lcl='!SI_CPUL!'.Trim(); $w=($lo,$lc,$lg,$lrf,$lrt,$lcl|Measure-Object Length -Maximum).Maximum+2; Write-Host; Write-Host '  ==========================================' -ForegroundColor DarkGray; Write-Host '    !SI_TITLE!' -ForegroundColor White; Write-Host '  ==========================================' -ForegroundColor DarkGray; Write-Host; $os=(Get-CimInstance Win32_OperatingSystem); $cpu=(Get-CimInstance Win32_Processor); $gpu=(Get-CimInstance Win32_VideoController|Select-Object -First 1); $cs=(Get-CimInstance Win32_ComputerSystem); Write-Host ('   '+$lo.PadRight($w)+$os.Caption) -ForegroundColor Gray; Write-Host ('   '+$lc.PadRight($w)+$cpu.Name) -ForegroundColor Gray; Write-Host ('   '+$lg.PadRight($w)+$gpu.Name) -ForegroundColor Gray; Write-Host ('   '+$lrf.PadRight($w)+[math]::Round($os.FreePhysicalMemory/1024)+' MB') -ForegroundColor Green; Write-Host ('   '+$lrt.PadRight($w)+[math]::Round($cs.TotalPhysicalMemory/1GB,1)+' GB') -ForegroundColor Gray; Write-Host ('   '+$lcl.PadRight($w)+$cpu.LoadPercentage+'%%') -ForegroundColor Yellow; Write-Host; Write-Host '  ==========================================' -ForegroundColor DarkGray; Write-Host; Write-Host '   [0]  !C_BACK!' -ForegroundColor DarkGray; Write-Host"
call :READ_KEY
goto MAIN_MENU
