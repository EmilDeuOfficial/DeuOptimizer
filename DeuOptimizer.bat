...
if "!CHOICE!"=="8" goto RESTORE_POINT
if "!CHOICE!"=="9" goto RESTORE_ALL
if /i "!CHOICE!"=="L" goto SHOW_LOG
...
:: ============================================================
cls
powershell -NoProfile -Command "$dw={param($s) $n=0; $s.ToCharArray()|%%{if([int]$_ -ge 0x1100){$n+=2}else{$n+=1}}; $n}; $lo='OS:'; $lc='CPU:'; $lg='!SI_GPU!'.Trim(); $lrf='!SI_RAMF!'.Trim(); $lrt='!SI_RAMT!'.Trim(); $lcl='!SI_CPUL!'.Trim(); $mw=(($lo,$lc,$lg,$lrf,$lrt,$lcl|%%{& $dw $_})|Measure-Object -Maximum).Maximum+2; $p={param($s) $s+(' '*($mw-(& $dw $s)))}; Write-Host; Write-Host '  ==========================================' -ForegroundColor DarkGray; Write-Host '    !SI_TITLE!' -ForegroundColor White; Write-Host '  ==========================================' -ForegroundColor DarkGray; Write-Host; $os=(Get-CimInstance Win32_OperatingSystem); $cpu=(Get-CimInstance Win32_Processor); $gpu=(Get-CimInstance Win32_VideoController|Select-Object -First 1); $cs=(Get-CimInstance Win32_ComputerSystem); Write-Host ('   '+(& $p $lo)+$os.Caption) -ForegroundColor Gray; Write-Host ('   '+(& $p $lc)+$cpu.Name) -ForegroundColor Gray; Write-Host ('   '+(& $p $lg)+$gpu.Name) -ForegroundColor Gray; Write-Host ('   '+(& $p $lrf)+[math]::Round($os.FreePhysicalMemory/1024)+' MB') -ForegroundColor Green; Write-Host ('   '+(& $p $lrt)+[math]::Round($cs.TotalPhysicalMemory/1GB,1)+' GB') -ForegroundColor Gray; Write-Host ('   '+(& $p $lcl)+$cpu.LoadPercentage+'%%') -ForegroundColor Yellow; Write-Host; Write-Host '  ==========================================' -ForegroundColor DarkGray; Write-Host; Write-Host '   [0]  !C_BACK!' -ForegroundColor DarkGray; Write-Host"
call :READ_KEY
goto MAIN_MENU
...
:SELECT_LANG
cls
powershell -NoProfile -EncodedCommand 
call :READ_KEY
set "LC=!INPUT_VAL!"
if "!LC!"=="1" ( set "LANG=EN" & (echo LANG=EN)>"!LANGFILE!" & goto :eof )
if "!LC!"=="2" ( set "LANG=DE" & (echo LANG=DE)>"!LANGFILE!" & goto :eof )
...
powershell -NoProfile -Command "$t='!_TEMP!'; Start-Sleep -Milliseconds 100; while([Console]::KeyAvailable){$null=[Console]::ReadKey($true)}; Write-Host -NoNewline '  > '; $k=[Console]::ReadKey($true); $c=[string]$k.KeyChar; if($c -match '[0-9CcSsLl]'){[System.IO.File]::WriteAllText($t,$c.ToUpper(),[System.Text.Encoding]::ASCII)}elseif($c -match '[Rr]'){Write-Host -NoNewline 'R'; $d=[Console]::ReadKey($true); [System.IO.File]::WriteAllText($t,'R'+([string]$d.KeyChar),[System.Text.Encoding]::ASCII)}"
set "INPUT_VAL="
if exist "!_TEMP!" (
